...
const FlightCalendar = ( { calDateSelect, day, month, year, setMonth } ) => {
    ...
    const generated = () => {
        const dayIndex = dayNames.indexOf( date().firstDayOfMonth( year, month-1, 1 ) );
        const fillStart = dayIndex;
        let arr = [];
                ...
                <div className="col d-flex flex-row justify-content-around">
                    {
                        dayNames.map( ( name, index ) => {
                            return (
                                <div className="col text-center p-1" key={`day_name_${name + index}`}>{name}</div>
                            )
                        } )
                    }
                </div>
